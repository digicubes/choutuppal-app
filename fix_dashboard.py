...
content = re.sub(r'\{formData\.logoUrl \? \([\s\S]*?className=\"size-4\" \/><\/button>\s*<\/>\s*\) : \(', logo_img, content)
...
content = re.sub(r'\{formData\.coverImage \? \([\s\S]*?className=\"size-4\" \/><\/button>\s*<\/>\s*\) : \(', cover_img, content)
