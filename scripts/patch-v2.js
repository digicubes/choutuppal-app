...
// 1. compressAndUpload
const oldCompressAndUploadRegex = /const compressAndUpload = async \(file: File, folder: string\) => \{[\s\S]*?return await res\.json\(\)\n  \}/;
...
// 2. submitListing error handling
content = content.replace("toast.error('Failed to create listing')", "const errData = await res.text(); console.error('Submit API error:', errData); alert('Failed to publish. Check console.'); toast.error('Failed to create listing')");
content = content.replace("toast.error('Something went wrong')", "console.error('Submit error:', err); alert('Failed to publish. Check console.'); toast.error('Something went wrong')");
content = content.replace("toast.success('Listing created successfully!')", "alert('Listing published successfully!'); toast.success('Listing created successfully!')");
content = content.replace("if (!currentUser || !formData.name || !formData.category) return\n    setUploading(true)", "if (!currentUser || !formData.name || !formData.category) return\n    console.log('Submitting:', formData)\n    setUploading(true)");


// 3. submitBanner error handling
content = content.replace("toast.error('Failed to create banner')", "const errData = await res.text(); console.error('Banner submit API error:', errData); toast.error('Failed to create banner')");

// 4. Lock Real Estate category
...
console.log('Patched dashboard-view.tsx for direct upload safely');
