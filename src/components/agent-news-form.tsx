...
export function AgentNewsForm({ onSuccess }: { onSuccess: () => void }) {
  const { user } = useAuth()
  ...
  const handleImageUpload = async (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    try {
      toast.loading('Uploading image...', { id: 'upload-news' })
      ...
      const { error } = await supabase.storage.from('listing-images').upload(`news/${fileName}`, compressedFile)
      if (error) throw error
      const { data: { publicUrl } } = supabase.storage.from('listing-images').getPublicUrl(`news/${fileName}`)
      
      setImageUrl(publicUrl)
      toast.success('Image uploaded successfully!', { id: 'upload-news' })
    } catch (error) {
      console.error("Upload error:", error)
      toast.error('Failed to upload image', { id: 'upload-news' })
    }
  }
  ...
  const handleSubmit = async () => {
    ...
    setLoading(true)
    try {
      // Find the agent's cityId. Fallback to temp if none (should have one).
      ...
      if (res.ok) {
        toast.success('News posted successfully! It is now live.')
        setTitle('')
        setContent('')
        setImageUrl('')
        onSuccess()
        ...
        toast.error(data.error || 'Failed to post news')
      }
    } catch (err) {
      console.error(err)
  ...
  return (
    <div className="space-y-6">
      <div>
        ...
        <Input placeholder="Enter news headline..." value={title} onChange={(e) => setTitle(e.target.value)} className="h-12 bg-gray-50 border-gray-200" />
      </div>
